    ...
    <form action="{{ route('productos.update', $producto) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="medicamento_id" id="medicamento_id"
                    ...
                    @foreach(\App\Models\Medicamento::orderBy('nombre')->get() as $med)
                        <option value="{{ $med->id }}" {{ old('medicamento_id', $producto->medicamento_id) == $med->id ? 'selected' : '' }}>
            ...
            <div class="md:col-span-2">
                <div class="bg-gray-50 border border-gray-200 rounded-lg p-4" id="datos-medicamento">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-700" id="info-nombre">{{ $producto->nombre }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400">Forma Farmacéutica</p>
                            <p class="text-sm font-medium text-gray-700" id="info-forma">{{ $producto->forma_farmaceutica ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400">Concentración</p>
                            <p class="text-sm font-medium text-gray-700" id="info-conc">{{ $producto->concentracion ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400">Precio Referencial</p>
                            <p class="text-sm font-medium text-gray-700" id="info-precio">{{ $producto->precio_referencial ? 'Bs. ' . number_format($producto->precio_referencial, 2) : '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="origen" value="{{ old('origen', $producto->origen) }}" placeholder="Ej: NAL, IMP"
            ...
            <div>
                ...
                <input type="text" name="marca" value="{{ old('marca', $producto->marca) }}" placeholder="Ej: IFA Prodexa"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
            </div>
