...
@section('content')
    ...
    <style>
        ...
        /* ── NUEVOS ESTILOS: Focus ring, linterna, zoom ── */
        .focus-ring {
            position: absolute;
            width: 60px;
            height: 60px;
        ...
        .focus-ring.activo {
            display: block;
            animation: focusFade 1s ease-out forwards;
        }
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="ocr-card p-6">
            <div class="flex items-center justify-between mb-5">
                ...
                <span id="estado-camara" class="text-xs font-medium text-gray-400 bg-gray-100 px-3 py-1 rounded-full">Sin
                    iniciar</span>
            </div>
            ...
            {{-- Webcam --}}
            <div id="modo-webcam">
                <div class="camara-box mb-4" id="camara-box-container">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    {{-- Botones linterna y zoom (solo aparecen si el dispositivo los soporta) --}}
                    <div class="flex gap-2" id="controles-extra" style="display:none!important">
                        <button onclick="toggleLinterna()" id="btn-linterna" class="btn-camara-extra btn-linterna-off">
                            🔦 Linterna
                        </button>
                        <button onclick="toggleZoom()" id="btn-zoom" class="btn-camara-extra btn-zoom-off">
                            🔍 Zoom ×2
                        </button>
                    </div>
            ...
            <div id="modo-archivo" class="hidden">
                ...
                <img id="preview-imagen" class="w-full rounded-xl hidden mb-3"
        ...
        <div class="ocr-card p-6">
            ...
            <div id="procesando" class="hidden flex flex-col items-center justify-center h-64 text-center">
                <div class="w-12 h-12 border-3 border-blue-200 border-t-blue-600 rounded-full animate-spin mb-4"
            ...
            <div id="resultados" class="hidden">
                <div class="mb-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    <div class="flex justify-between items-center mb-1.5">
                        <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Precisión de lectura
                            IA</span>
                        <span id="texto-porcentaje" class="text-sm font-black text-blue-600">0%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div id="barra-porcentaje"
                            class="bg-blue-600 h-2 rounded-full transition-all duration-1000 ease-out" style="width: 0%">
                        </div>
                    </div>
        ...
        <div class="overflow-x-auto rounded-xl border border-gray-100">
            <table class="w-full text-sm text-left">
                ...
                <tbody id="historial-body">
                    <tr>
                        <td colspan="4" class="px-5 py-8 text-center text-gray-300 text-xs">No hay detecciones en esta
                            sesión</td>
                    </tr>
                </tbody>
            </table>
    ...
    <div id="modal-manual" class="modal-overlay">
        <div class="modal-box">
            ...
            <div class="space-y-3 mb-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase mb-1.5">💊 Nombre del
                        medicamento</label>
            ...
            <div class="bg-red-50 border border-red-100 rounded-xl p-4 mb-5">
                <div class="grid grid-cols-2 gap-3 text-sm mb-4">
                    <div>
                        ...
                        <p id="vencido-producto" class="font-semibold text-gray-800">-</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-0.5">Lote</p>
                        <p id="vencido-lote" class="font-semibold text-gray-800">-</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-0.5">Cantidad</p>
                        <p id="vencido-cantidad" class="font-semibold text-red-600">-</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-0.5">Vencimiento</p>
                        <p id="vencido-fecha" class="font-semibold text-red-600">-</p>
                    </div>
                </div>
                <div>
...
@push('scripts')
    <script>
        ...
        // ── Nuevas variables para cámara avanzada ──
        ...
        async function iniciarWebcam() {
            try {
                ...
                // Guardar track para linterna, zoom y enfoque
                trackActual = stream.getVideoTracks()[0];

                // Mostrar controles extra según capacidades del dispositivo
                const capabilities = trackActual.getCapabilities ? trackActual.getCapabilities() : {};
                const controlesExtra = document.getElementById('controles-extra');
                if (capabilities.torch || capabilities.zoom) {
                    controlesExtra.style.removeProperty('display');
                    controlesExtra.style.display = 'flex';
                }
                if (!capabilities.torch) document.getElementById('btn-linterna').style.display = 'none';
                if (!capabilities.zoom) document.getElementById('btn-zoom').style.display = 'none';

                document.getElementById('btn-iniciar').disabled = true;
                document.getElementById('btn-detener').disabled = false;
                document.getElementById('btn-detener').className =
                    'flex-1 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-xl text-sm px-4 py-2.5 transition-all';
                document.getElementById('estado-camara').textContent = '🟢 Activa';
                document.getElementById('estado-camara').className =
                    'text-xs font-medium text-green-600 bg-green-50 px-3 py-1 rounded-full';

                video.onloadedmetadata = () => {
                    video.play();
                    document.getElementById('btn-capturar-manual').disabled = false;
                    document.getElementById('guia-badge').textContent = '📷 Cámara lista · Toque para enfocar';
                    document.getElementById('guia-badge').classList.add('visible');
                };

                // Tap to focus
                document.getElementById('camara-box-container').addEventListener('click', enfocarEnPunto);
        ...
        function detenerWebcam() {
            ...
            if (linternaActiva && trackActual) {
                try {
                    trackActual.applyConstraints({
                        advanced: [{
                            torch: false
                        }]
                    });
            ...
            // Resetear botones extra
            document.getElementById('controles-extra').style.display = 'none';
            document.getElementById('btn-linterna').textContent = '🔦 Linterna';
            document.getElementById('btn-linterna').className = 'btn-camara-extra btn-linterna-off';
            document.getElementById('btn-zoom').textContent = '🔍 Zoom ×2';
            document.getElementById('btn-zoom').className = 'btn-camara-extra btn-zoom-off';
            zoomActivo = false;
            trackActual = null;

            // Remover listener de enfoque
            const box = document.getElementById('camara-box-container');
            box.removeEventListener('click', enfocarEnPunto);
            ...
            document.getElementById('btn-detener').className =
                'flex-1 bg-gray-200 text-gray-400 font-semibold rounded-xl text-sm px-4 py-2.5 transition-all';
            document.getElementById('estado-camara').textContent = 'Detenida';
            document.getElementById('estado-camara').className =
                'text-xs font-medium text-gray-400 bg-gray-100 px-3 py-1 rounded-full';
        }
        ...
        async function toggleLinterna() {
            ...
            try {
                await trackActual.applyConstraints({
                    advanced: [{
                ...
                if (linternaActiva) {
                    btn.textContent = '🔦 Linterna ON';
                    btn.className = 'btn-camara-extra btn-linterna-on';
                } else {
                    btn.textContent = '🔦 Linterna';
                    btn.className = 'btn-camara-extra btn-linterna-off';
                }
            } catch (e) {
                alert('Este dispositivo no soporta control de linterna.');
                linternaActiva = false;
            }
        }
        ...
        async function toggleZoom() {
            ...
            try {
                const capabilities = trackActual.getCapabilities();
                const zoomVal = zoomActivo ?
                    Math.min(capabilities.zoom.max, capabilities.zoom.min * 2) :
                ...
                if (zoomActivo) {
                    ...
                    btn.className = 'btn-camara-extra btn-zoom-on';
                } else {
                    btn.textContent = '🔍 Zoom ×2';
                    btn.className = 'btn-camara-extra btn-zoom-off';
                }
            } catch (e) {
                alert('Este dispositivo no soporta zoom por software.');
                zoomActivo = false;
            }
        }

        // ── Tap to Focus ──
        async function enfocarEnPunto(e) {
            // No interferir con botones dentro del box
            ...
            ring.classList.remove('activo');
            void ring.offsetWidth; // forzar reflow para reiniciar animación
            ring.classList.add('activo');
            clearTimeout(focusTimeout);
            focusTimeout = setTimeout(() => ring.classList.remove('activo'), 1000);

            // Aplicar enfoque real si el dispositivo lo soporta
            try {
                const capabilities = trackActual.getCapabilities ? trackActual.getCapabilities() : {};
                if (capabilities.focusMode && capabilities.pointsOfInterest) {
                    ...
                    // Volver a continuo después de 2.5s
                    setTimeout(async () => {
                        try {
                            await trackActual.applyConstraints({
                                advanced: [{
                ...
                // Dispositivo no soporta enfoque manual — solo efecto visual
            }
        }
        ...
        function mostrarResultado(nombre, fecha, estado, lote) {
            ...
            nombreEl.className = nombre ? 'valor text-sm flex flex-wrap items-center gap-2 mt-1' :
                'valor vacio text-sm mt-1';
            if (lote) {
                nombreEl.innerHTML =
                    `<span>${nombre ?? 'Sin nombre'}</span> <span class="text-[10px] bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full font-bold tracking-wide border border-blue-200 shadow-sm whitespace-nowrap">LOTE: ${lote}</span>`;
            } else {
        ...
        async function buscarPorLote(lote, fecha, estadoOcr) {
            try {
                ...
                if (data.encontrado) {
                    ...
                    actualizarBarraPrecision(40);
                    mostrarResultado(null, fecha, estadoOcr, lote);
                    document.getElementById('resultado-nombre').innerHTML =
                        `<span class="text-orange-500 text-sm">Lote ${lote} no encontrado en la lista de productos</span>`;
                }
            } catch (e) {
        ...
        function guardarDeteccion() {
            ...
            console.log('Enviando:', ultimaDeteccion);
            fetch('/ocr/guardar', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    nombre: ultimaDeteccion.nombre,
                    fecha: ultimaDeteccion.fecha,
                    estado: ultimaDeteccion.estado,
                    lote: ultimaDeteccion.lote
                })
        ...
        function guardarDesdeModal() {
            ...
            if (!nombre && !fecha) {
                alert('Ingresa al menos el nombre o la fecha.');
                return;
            }
        ...
        function limpiarResultados() {
            ...
            document.getElementById('badge-estado-resultado').className =
                'text-xs font-medium text-gray-400 bg-gray-100 px-3 py-1 rounded-full';
        ...
        function agregarAlHistorial(nombre, fecha, estado) {
            ...
            const ahora = new Date().toLocaleDateString('es-BO') + ' ' + new Date().toLocaleTimeString('es-BO', {
                hour: '2-digit',
                minute: '2-digit'
            });
            const estadoClase = {
                'VIGENTE': 'bg-green-50 text-green-700',
                'PROXIMO': 'bg-yellow-50 text-yellow-700',
